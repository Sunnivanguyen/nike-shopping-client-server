...
const PersonalInfomation = ({ closeModal }) => {
  const { user, updateUser, fetchUsers, userAddresses } = useUser();
  ...
  const addressDefault =
    ...
    userAddresses.filter((el: IUserAddress) => el.is_default);
  ...
  async function fetchData(dataUpdate) {
    await updateUser(user.id, dataUpdate);
    await fetchUsers();
  }
